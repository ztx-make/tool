...
options() {
    ...
    if [ -z "$1" ]; then
        if [ -z "$ZTX_MAKE_EXTENSION_SERIES" ]; then
            echoFatal "tar file path missing"
        else
            if [ -f "/origin/series-$ZTX_MAKE_EXTENSION_SERIES/artifact/source.repo.tar" ]; then
                sourceTarFile="/origin/series-$ZTX_MAKE_EXTENSION_SERIES/artifact/source.repo.tar"
                sourceVersionControl="repo"
            else
                sourceTarFile="/origin/series-$ZTX_MAKE_EXTENSION_SERIES/artifact/source.git.tar"
            fi
        fi
    else
